...
def test_bil_full_loan_flow(shared_page):
    ...
    base.renoverings_loan()

    pu = Personuppgifter(shared_page)
    data = pu.personuppgifter_form()
    expect(shared_page.get_by_role("textbox", name="Personnummer *")).to_have_value(data["pnr"])  # Kontrollera att personnumret är ifyllt med skattaverkets test personnummer.
    expect(shared_page.get_by_role("textbox", name="Förnamn")).to_have_value(data["first_name"])  # Kontrollera att förnamnet är ifyllt med ett fake namn.
    expect(shared_page.get_by_role("textbox", name="Efternamn")).to_have_value(data["last_name"])  # Kontrollera att efternamnet är ifyllt med ett fake efternamn.
    ...
    loan_data = lp.laneuppgifter_form(amount=500000)
